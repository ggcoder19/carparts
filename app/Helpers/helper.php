...
function countMakeProducts2($b, $m, $s, $p, $sp, $pi){
    //echo $make;
    //echo '<br/>'.$partCategoryId;
    $query = "SELECT count(*) as totalProducts FROM products WHERE brand_id = '$b' AND car_model = '$m' AND car_submodel = '$s' and is_part = '$p' AND sub_part = '$sp' AND FIND_IN_SET('$pi', pieces) ";
...
function getTotalProducts($b, $m, $s, $p){
    $query = "SELECT count(*) as totalProducts FROM products WHERE brand_id = '$b' and car_model = '$m' and car_submodel = '$s' and is_part = '$p'";
    $results = DB::select( DB::raw($query) );
    //dd($results);
    return $results[0]->totalProducts;
}